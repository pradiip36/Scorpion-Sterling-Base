...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="/">
        <xsl:element name="MultiApi">
            <xsl:apply-templates select="/MultiApi/PathLists/PathList[@Type='CopyCalendar']"/>
        </xsl:element>
    </xsl:template>
    <xsl:template match="PathList">
        ...
        <xsl:variable name="modelOrg" select="/MultiApi/API/Output/Calendar[@OrganizationCode=$modelOrgKey]"/>
        <xsl:element name="API">
            <xsl:attribute name="Name"><xsl:value-of select="'createCalendar'"/></xsl:attribute>
