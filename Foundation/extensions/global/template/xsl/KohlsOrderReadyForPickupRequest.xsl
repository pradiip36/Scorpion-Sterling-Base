...
<xsl:stylesheet version="1.0" 
    ...
    <web:order xmlns:web='http://webservice.services.cc.ch.kohls.com'>
		...
		<xsl:for-each select="/Order/OrderLines/OrderLine">
			<web:products>
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<web:skuPacklistTyp><xsl:text disable-output-escaping="yes">&#160;</xsl:text></web:skuPacklistTyp>
					</xsl:otherwise>
				</xsl:choose>
		...
		<web:campaign xmlns:web='http://webservice.services.cc.ch.kohls.com'>
			<web:folderName>Order_Ready</web:folderName>
			<web:objectName>Order_Ready_Test</web:objectName>
		</web:campaign>
