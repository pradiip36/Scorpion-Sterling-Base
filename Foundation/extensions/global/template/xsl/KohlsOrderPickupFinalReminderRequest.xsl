...
<xsl:stylesheet version="1.0" 
    ...
    <web:order xmlns:web='http://webservice.services.cc.ch.kohls.com'>
		...
		<web:pickupTime></web:pickupTime>
		<web:orderReceiptId><xsl:value-of select="Order/@ReceiptID"/></web:orderReceiptId><!--dateTime-->
		...
		<xsl:for-each select="/Order/OrderLines/OrderLine">
			<web:products>
				...
				<xsl:choose>
					<xsl:when test="@PackListType != ''">
						<web:skuPacklistTyp><xsl:value-of select="@PackListType"/></web:skuPacklistTyp>
					</xsl:when>
					<xsl:otherwise>
						<web:skuPacklistTyp><xsl:text> </xsl:text></web:skuPacklistTyp>
					</xsl:otherwise>
				</xsl:choose>
		...
		<web:campaign xmlns:web='http://webservice.services.cc.ch.kohls.com'>
			<web:folderName>Final_Pickup_Reminder</web:folderName>
			<web:objectName>TEST_201403_Final_Pickup_Reminder</web:objectName>
		</web:campaign>
