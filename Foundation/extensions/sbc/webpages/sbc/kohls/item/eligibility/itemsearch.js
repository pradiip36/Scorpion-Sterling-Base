...
sc.sbc.kohls.item.eligibility.ItemSearch = function(config) {
	config = config || {};
	Ext.applyIf(config, {
                        sciId : "itemSearch"
                  });
	sc.sbc.kohls.item.eligibility.ItemSearch.superclass.constructor.call(this, config);
}
Ext.extend(sc.sbc.kohls.item.eligibility.ItemSearch, sc.sbc.common.search.sbcsearch, {
	    ...
	    basicSearch : {
				...
				getTitle : function() {
					return "Set Eligible Items for Store";
				},
				getViewID : function() {
...
Ext.reg('seaItemSearch4', sc.sbc.kohls.item.eligibility.ItemSearch);
