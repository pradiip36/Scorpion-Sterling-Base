...
sc.sbc.kohls.item.eligibility.BasicItemSearch = function(config) {
	config = config || {};
	if(config.scOwnerScr && config.scOwnerScr.hideSearchServiceOnlyOption == false){
		this.hideSearchServiceOnlyOption = false;
	}
	sc.sbc.kohls.item.eligibility.BasicItemSearch.superclass.constructor.call(this, config);
	/*if(config.scOwnerScr && config.scOwnerScr.SearchInput){
		var searchData = {};
		searchData.SearchWrapper = {};
		searchData.SearchWrapper.Item = config.scOwnerScr.SearchInput.Item;
		this.paintSavedSearch(searchData);
	}*/
}
Ext.extend(sc.sbc.kohls.item.eligibility.BasicItemSearch, sc.plat.ui.ExtensibleScreen, {
	className : 'sc.sbc.kohls.item.eligibility.BasicItemSearch',
	namespaces : {
		source : ["itemSearch_output", "SavedSearch", "ItemTypeList_output"],
		target : ["itemSearch_input", "itemDetails_input"]
	},
	
	namespacesDesc: {

        sourceDesc: ["Not used","This namespace contains the saved search model used to populate saved searches."
        ...
        targetDesc: ["This namespace contains the input to getItemList API get the search results",
                     "Not used"]

    },
	getUIConfig : sc.sbc.kohls.item.eligibility.BasicItemSearchUIConfig,
	...
	getScreenTargetModel: function(config){
		...
		var categoryTrigger = this.find("sciId", "triggerItemSearchCategory")[0];
	    if(categoryTrigger && !categoryTrigger.getValue()){
	    	searchInput	.Item.CategoryFilter={};
	    }
		if(searchInput.Item && this.scOwnerScr.apiInput && this.scOwnerScr.apiInput.Item){
			sc.sbc.util.JsonUtils.mergeObjects(searchInput.Item, this.scOwnerScr.apiInput.Item);
		}
	...
	alert(searchData);
		this.disableFields({disable:false, allowCopy:false});
		this.setModel(searchData, "SavedSearch", {clearOldVals :true});
    },
	...
	initialize : function(){
		...
		if(this.scOwnerScr && this.scOwnerScr.SearchInput){
			sc.sbc.util.JsonUtils.mergeObjects(searchCriteria.SearchWrapper.Item, this.scOwnerScr.SearchInput.Item);
		}
		if(this.scOwnerScr && this.scOwnerScr.apiInput && this.scOwnerScr.apiInput.Item){
			//Merging the jsons to populate in the UI.
			sc.sbc.util.JsonUtils.mergeObjects(searchCriteria.SearchWrapper.Item, this.scOwnerScr.apiInput.Item);
			
			//Disabling based on 'this.scOwnerScr.apiInput' json
			...
			if(this.scOwnerScr.apiInput.Item.OnlyIncludeMyCatalog === "Y" 
				...
				var chkItemSearchCatalog = this.find("sciId", "chkItemSearchCatalog")[0];
				chkItemSearchCatalog.disable();
			}
			
			
			if(itemInput.CategoryFilter && itemInput.CategoryFilter.CategoryPath && itemInput.CategoryFilter.CategoryPathQryType === "NE"){
				var categoryTrigger = this.find("sciId", "triggerItemSearchCategory")[0];
				categoryTrigger.disable();
				
				searchCriteria.SearchWrapper.Item.CategoryFilter.CategoryPath = "";
			}
			//End - Disabling based on 'this.scOwnerScr.apiInput' json
		}
		
		this.setModel(itemManager.getItemTypeList(), "ItemTypeList_output", {clearOldVals :true});
		this.setModel(searchCriteria, "SavedSearch", {clearOldVals :true});
	},
	...
	handleCategoryResult : function(categoryArray){
	    ...
	    var categoryPathField = this.find("sciId", "sbcCategoryPath")[0];
	...
	showResultText : function (btn, text){
        //Ext.msg('Button Click', 'You clicked the {0} button and entered the text "{1}".', btn, text);
    },

    selectVendor: function(){
    	
		var vendorPopUp = new sc.sbc.kohls.item.eligibility.VendorSearch();
...
Ext.reg('seaBasicItemSearch3', sc.sbc.kohls.item.eligibility.BasicItemSearch);
