...
public class KohlsSendShipAloneToWMoSAgent extends YCPBaseTaskAgent {

	private static final YFCLogCategory log = YFCLogCategory.instance(KohlsSendShipAloneToWMoSAgent.class.getName());
	private YIFApi api;


	public KohlsSendShipAloneToWMoSAgent() throws YIFClientCreationException {
	...
	throws Exception {
		...
		try{
		//Order Broker end
		if (KohlsXMLLiterals.A_SHIPMENT_KEY.equalsIgnoreCase(sDataType)) {
			...
			yfcEleGetShipmentDetails.setAttribute(KohlsXMLLiterals.A_SHIPMENT_KEY, sDataKey);			
			...
			//System.out.println("GetShipment Details Output XML : " + XMLUtil.getXMLString(docOutputGetShipmentDetails));
			...
			for (int i = 0; i < ndlstShipmentLines.getLength(); i++) {
				...
				if(!strOrderLineKey.equals("")){
					...
					//System.out.println("getOrderReleaseDetailsTemplate Output XML : " + XMLUtil.getXMLString(docOutputGetOrderLineDetails));
				...
				if(inNumOfNotes>0){
					...
					if((KohlsConstant.PICK).equalsIgnoreCase(sDeliveryMethod)){ //BOPUS line
						//Prefixing constant "BPS_" to the shipnode
			...
			this.changeShipmentStatus(env, sDataKey);
...
/*			if(isSingleLine && strExtWrapCnt==1){
			...
			//System.out.println("Release Message to WMoS : " + XMLUtil.getXMLString(yfcDocOrderRelease.getDocument()));
			if (YFCLogUtil.isDebugEnabled()) {
				log.debug("Release Status Message To Ecommerce : " + XMLUtil.getXMLString(yfcDocOrderStatusToECommXML.getDocument()));
			}
			this.api.executeFlow(env, KohlsConstant.SERVICE_KOHLS_SEND_SHIP_ALONE_STATUS_TO_ECOMM, yfcDocOrderStatusToECommXML.getDocument());
			//System.out.println("Release Status Message To Ecommerce : " + XMLUtil.getXMLString(yfcDocOrderStatusToECommXML.getDocument()));
		}
	...
	private void changeShipmentStatus(YFSEnvironment env, String sShipmentKey) throws YFSException, RemoteException{
		...
		yfcEleChangeShipmentStatus.setAttribute(KohlsXMLLiterals.A_SHIPMENT_KEY, sShipmentKey);
		yfcEleChangeShipmentStatus.setAttribute(KohlsXMLLiterals.A_TRANSACTIONID, KohlsConstant.TRAN_ID_SA_SEND_RELEASE_TO_WMOS);

		
		
		if (YFCLogUtil.isDebugEnabled()) {
			log.debug("Input XML to changeShipmentStatus : " + XMLUtil.getXMLString(yfcDocChangeShipmentStatus.getDocument()));
		}
		this.api.changeShipmentStatus(env, yfcDocChangeShipmentStatus.getDocument());
	}
