 ...
 *
 */
public class KOHLSPopulateTaxPackShip {


	private static final YFCLogCategory log = YFCLogCategory.instance(
			KOHLSPopulateTaxPackShip.class.getName());
	 ...
	 * 					e
	 */
	public Document populateTaxPack(YFSEnvironment yfsEnvironment,
			Document inputDoc) throws Exception	{

		if (YFCLogUtil.isDebugEnabled()) {
			
			this.log.debug("<!-- Begining of KOHLSPopulateTaxPackShip" +
					" populateTaxPack method -- >"
		...
		if(!YFCObject.isVoid(inputDoc)) {
			...
			try{
				...
				Element eleContainer = (Element) eleInputDoc.getElementsByTagName(KohlsConstant.E_CONTAINERS).item(0);
				NodeList nlShipmenLine = eleContainer.getElementsByTagName(KohlsConstant.E_SHIPMENT_LINE);
				...
				this.log.debug("<!-- Exception in KOHLSPopulateTaxPackShip" +
						" populateTaxPack method -- >");
				e.printStackTrace();
			}
		...
		if (YFCLogUtil.isDebugEnabled()) {
			...
			this.log.debug("<!-- End of KOHLSPopulateTaxPackShip" +
					" populateTaxPack method -- >"
					+ XMLUtil.getXMLString(inputDoc));
		}
