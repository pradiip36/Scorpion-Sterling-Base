...
public class KOHLSShipConfirmMsg implements YIFCustomApi {

	private YIFApi api;

	private static final YFCLogCategory log = YFCLogCategory
			.instance(KOHLSShipConfirmMsg.class.getName());
	 ...
	 *             e
	 */
	public KOHLSShipConfirmMsg() throws YIFClientCreationException {

		this.api = YIFClientFactory.getInstance().getLocalApi();
	}

	public Document massageMsg(YFSEnvironment env, Document inXML)
			throws Exception {

		YFCDocument inDoc = YFCDocument.getDocumentFor(inXML);
		YFCElement inEle = inDoc.getDocumentElement();
		log
				.debug("***************inside massageMsg mehtod of KOHLSShipConfirmMsg class **********");
