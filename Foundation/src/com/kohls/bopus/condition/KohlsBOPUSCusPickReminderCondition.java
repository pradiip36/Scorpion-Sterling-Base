...
public class KohlsBOPUSCusPickReminderCondition implements YCPDynamicConditionEx {
	...
	public boolean evaluateCondition(YFSEnvironment env, String arg1,
		...
		if(YFCLogUtil.isDebugEnabled()){
			log.debug("******** Input to KohlsBOPUSCusPickReminderCondition *************"+XMLUtil.getXMLString(inXML));
		}
		...
		try{
			...
			if(strStatus.equalsIgnoreCase(KOHLSCommonCodeList.statusValue(env,KohlsConstant.CC_DESC_READY_FOR_CUSTOMER, KohlsConstant.CODE_TYPE_SHP))){
				...
				String cutOffDay = getProperty(KohlsConstant.PICKUP_REMINDER_DAYS);
