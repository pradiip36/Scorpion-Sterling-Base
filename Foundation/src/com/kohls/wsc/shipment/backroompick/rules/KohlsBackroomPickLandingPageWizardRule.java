...
package com.kohls.wsc.shipment.backroompick.rules;
...
public class KohlsBackroomPickLandingPageWizardRule implements ISCUIWizardRule {
	public String execute(SCUIWizardRuleContext wizardRuleContext,
		...
		if (!(SCUtil.isVoid(status))
				&& (("1100.70.06.10".equals(status)
						|| ("1100.70.06.20".equals(status)) 
						|| ("1100.70.06.50.2".equals(status)) 
						|| ("1100.70.06.50.4".equals(status))
						|| ("1100.70.06.30".equals(status))
						|| ("1400".equals(status)) 
						|| ("1400.1".equals(status)) && (!(SCUtil.isVoid(screen))) && (!("ItemScan"
							.equals(screen)))))) {
